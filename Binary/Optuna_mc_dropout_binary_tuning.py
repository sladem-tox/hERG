...
parser = argparse.ArgumentParser(description="Load training data for active learning model.")
parser.add_argument('trainfile', type=str, help='Path to the training CSV data file')
parser.add_argument("--target", type=str, default="f1", choices=["f1", "balanced_accuracy"],
                    help="Metric to optimize")
args = parser.parse_args()

# Load training data
train_data = pd.read_csv(args.trainfile)
X = train_data.drop(columns=['COMPOUND_ID', 'target']).values
y = train_data['target'].values

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# ---- MC_dropout model parameterized ----
...
def objective(trial):
    ...
    # Balanced accuracy, F1 or ROC_AUC can be used
    if args.target == "f1":
        score = f1_score(y_val, y_pred_bin)
    elif args.target == "balanced_accuracy":
        score = balanced_accuracy_score(y_val, y_pred_bin)
...
print(f"Best {args.target}:", study.best_value)

# Save for later use in Active Learning
import json, os
os.makedirs("results", exist_ok=True)
filename = f"best_params_{args.target}.json"
with open(os.path.join("results", filename), "w") as f:
    json.dump(study.best_params, f, indent=2)

print(f"Saved best params for {args.target} to {filename}")
